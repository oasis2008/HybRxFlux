...
/**
 * 通话界面功能控制按钮操作区域 com.yuntongxun.ecdemo.ui.voip in ECDemo_Android Created by
 * Jorstin on 2015/7/3.
 */
public class ECCallControlUILayout extends LinearLayout implements
    ...
    public void setControlEnable(boolean enable) {
        ...
        mCallTransfer.setEnabled(enable);
    }
	...
	private OnClickListener l = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if(v.getId()==R.id.layout_call_mute){
				VoIPCallHelper.setMute();
                boolean mute = VoIPCallHelper.getMute();
                mCallMute
				.setImageResource(mute ? R.drawable.mute_icon_on : R.drawable.mute_selector);
				
			}else if(v.getId()==R.id.layout_call_handfree){
				VoIPCallHelper.setHandFree();
                boolean handFree = VoIPCallHelper.getHandFree();
                mCallHandFree .setImageResource(handFree ? R.drawable.handsfree_icon_on : R.drawable.handfree_selector);
			}
    ...
    public void onClick(View v) {
        ...
        if(v.getId()==R.id.layout_call_accept){
        	mOnCallControlDelegate.onViewAccept(this, view);
        	
        }else if(v.getId()==R.id.layout_call_cancel){
        	mOnCallControlDelegate.onViewReject(this , view);
        	
        }else if(v.getId()==R.id.layout_call_release){
        	mOnCallControlDelegate.onViewRelease(this, view);
