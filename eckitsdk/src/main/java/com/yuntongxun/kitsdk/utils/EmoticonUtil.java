...
public class EmoticonUtil {
	...
	public static void initEmoji() {
		...
		String[] stringArray = CCPAppManager.getContext().getResources()
		...
		for(String emojiUnicode : emojiUnicodes) {
			...
			if(charArray != null && charArray.length > 0) {
				...
				if (emojiResid != -1) {
					...
					LogUtil.v("[CCPUtils.initEmojiIcons] get a icon by name: " + emojiUnicode + " , resid: " + emojiResid);
	...
	public static int getEmojiResid(int num) {
		return getEmoticonResId(num + "", CCPAppManager.getContext());
	}
	...
	public void initEmojiIcons() {
		...
		if (fields != null) {
			LogUtil.d("[CCPUtils.initEmojiIcons] get all icons by reflect: " + fields.length);
			CCPEmoji emojEentry;
			for (Field field : fields) {
				try {
					...
					if (name.startsWith("emoji_") && emojiFilter(name)) {
						...
						LogUtil.v("[CCPUtils.initEmojiIcons] get a icon by name: " + name);
